...
class SwiftyBeaverTests: XCTestCase {
    ...
    func testVersionAndBuild() {
        XCTAssertGreaterThan(SwiftyBeaverClass.version.length, 4)
        XCTAssertGreaterThan(SwiftyBeaverClass.build, 500)
    }

    func testStripParams() {
        var f = "singleParam"
        XCTAssertEqual(SwiftyBeaverClass.stripParams(function: f), "singleParam()")
        f = "logWithParamFunc(_:foo:hello:)"
        XCTAssertEqual(SwiftyBeaverClass.stripParams(function: f), "logWithParamFunc()")
        f = "aFunc()"
        XCTAssertEqual(SwiftyBeaverClass.stripParams(function: f), "aFunc()")
    }
